    ...
    <div style={{ minHeight: '100vh', color: '#EAE8F4' }}>
      <div style={{ maxWidth: 720, margin: '0 auto', padding: 'clamp(40px,6vw,88px) clamp(20px,5vw,40px)' }}>
        ...
        <div style={{ fontSize: 15, color: '#B8B5D1', lineHeight: 1.9 }}>
          ...
          <h2 style={{ fontFamily: serif, fontSize: 24, fontWeight: 700, color: '#F3F1FB', margin: '40px 0 14px' }}>Why Visio is the wrong tool for VSM</h2>
          ...
          <h2 style={{ fontFamily: serif, fontSize: 24, fontWeight: 700, color: '#F3F1FB', margin: '40px 0 14px' }}>The top free VSM tools compared</h2>
          ...
          {[
            ...
            <div key={tool.name} style={{
              background: tool.highlight ? 'rgba(212,162,8,0.06)' : 'rgba(8,8,24,0.78)',
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12, flexWrap: 'wrap', gap: 8 }}>
                <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                  <span style={{ fontWeight: 800, fontSize: 17, color: tool.highlight ? '#D4A208' : '#F3F1FB' }}>{tool.name}</span>
                  {tool.highlight && <span style={{ fontSize: 9, background: '#D4A208', color: '#03030D', padding: '2px 8px', borderRadius: 999, fontWeight: 800, letterSpacing: 1 }}>RECOMMENDED</span>}
                </div>
                <span style={{ fontSize: 13, color: tool.highlight ? '#D4A208' : '#8B88B3', fontFamily: 'monospace' }}>{tool.price}</span>
              </div>
              <p style={{ fontSize: 12, color: '#8B88B3', marginBottom: 14 }}>Best for: {tool.best}</p>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  <div style={{ fontSize: 11, color: '#1DD1A1', fontWeight: 700, marginBottom: 8, letterSpacing: 1 }}>PROS</div>
                  {tool.pros.map((p, i) => <div key={i} style={{ fontSize: 12, color: '#8B88B3', marginBottom: 5, lineHeight: 1.5 }}>✓ {p}</div>)}
                </div>
                <div>
                  <div style={{ fontSize: 11, color: '#FF6B6B', fontWeight: 700, marginBottom: 8, letterSpacing: 1 }}>CONS</div>
                  {tool.cons.map((c, i) => <div key={i} style={{ fontSize: 12, color: '#8B88B3', marginBottom: 5, lineHeight: 1.5 }}>✗ {c}</div>)}
                </div>
              </div>
            </div>
          ))}

          <h2 style={{ fontFamily: serif, fontSize: 24, fontWeight: 700, color: '#F3F1FB', margin: '40px 0 14px' }}>The bottom line</h2>
          <p style={{ marginBottom: 18 }}>
            If you just need a quick diagram to share with your team, draw.io is free and functional. If you want a tool that actually <em style={{ color: '#F3F1FB' }}>understands lean</em> — calculates your lead time, connects to kaizen tracking, and lets AI help you build maps from SOPs — VeSiMy is the only free option that does all of that.
          </p>
