    ...
    <div style={{ minHeight: '100vh', background: '#F8F6F0', fontFamily: 'Arial, sans-serif' }}>
      ...
      <div style={{ background: '#242220', padding: 'clamp(48px,6vw,80px) clamp(16px,4vw,48px)', textAlign: 'center', borderBottom: '3px solid #0B1D33' }}>
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap' }}>
          {DEMOS.map(d => (
            <span key={d.slug} style={{ fontSize: 11, padding: '4px 12px', borderRadius: 100, background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.15)', color: 'rgba(248,247,245,0.6)', fontFamily: 'var(--font-mono)', letterSpacing: 0.5 }}>
              {d.icon} {d.industry}
            </span>
          ))}
        </div>
          ...
          {DEMOS.map((demo, i) => (
            <div key={demo.slug} style={{ background: '#FFFFFF', border: '0.5px solid #D8D5CE', borderRadius: 16, overflow: 'hidden', display: 'grid', gridTemplateColumns: '1fr auto', gap: 0 }}>
              <div style={{ padding: 'clamp(24px,3vw,36px)', borderRight: '0.5px solid #D8D5CE' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
                  ...
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3 }}>
                      <span style={{ fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 100, background: demo.tagBg, color: demo.tagColor, fontFamily: 'var(--font-mono)', letterSpacing: 0.5 }}>{demo.industry}</span>
                      <span style={{ fontSize: 11, color: '#8E8A82', fontFamily: 'var(--font-mono)' }}>Demo {String(i + 1).padStart(2, '0')}</span>
                    </div>
                ...
                <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', marginBottom: 16 }}>
                  <div style={{ padding: '8px 12px', background: 'rgba(192,64,42,0.06)', border: '1px solid rgba(192,64,42,0.2)', borderRadius: 8 }}>
                    <div style={{ fontSize: 9, color: '#C0402A', fontFamily: 'var(--font-mono)', letterSpacing: 1, marginBottom: 2 }}>BOTTLENECK</div>
                  ...
                  <div style={{ padding: '8px 12px', background: '#F8F6F0', border: '1px solid #D8D5CE', borderRadius: 8 }}>
                    <div style={{ fontSize: 9, color: '#8E8A82', fontFamily: 'var(--font-mono)', letterSpacing: 1, marginBottom: 2 }}>LEAD TIME</div>
                ...
                <div style={{ marginBottom: 16 }}>
                  <div style={{ fontSize: 10, color: '#8E8A82', fontFamily: 'var(--font-mono)', letterSpacing: 1, marginBottom: 8, textTransform: 'uppercase' }}>Tools included</div>
              ...
              <div style={{ padding: '36px 28px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12, minWidth: 180, textAlign: 'center', background: '#FAFAF8' }}>
                <div style={{ fontSize: 11, fontWeight: 800, fontFamily: 'var(--font-mono)', color: demo.color, padding: '4px 10px', background: `${demo.color}18`, border: `1px solid ${demo.color}35`, borderRadius: 6, display: 'inline-block', letterSpacing: 1, marginBottom: 8 }}>{demo.industry.split(' ')[0]}</div>
