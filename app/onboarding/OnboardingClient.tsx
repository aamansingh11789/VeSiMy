          ...
          {Array.from({ length: TOTAL_STEPS }, (_, i) => i + 1).map(n => (
            <div key={n} style={{ display:'flex', alignItems:'center', gap:8 }}>
              <div style={{
                ...
                border: step > n ? '1.5px solid #2E844A' : step === n ? '1.5px solid #C9A66B' : '1.5px solid var(--border)',
        ...
        <div style={{ width:'100%', maxWidth:680 }}>
            ...
            <div>
              ...
              <div style={{ display:'flex', flexWrap:'wrap', gap:6, marginBottom:20 }}>
                <button onClick={() => setSector('')} style={{
                  ...
                  borderColor: sector==='' ? '#C9A66B' : 'var(--border)',
                ...
                {INDUSTRY_SECTORS.map(s => (
                  <button key={s} onClick={() => setSector(s)} style={{
                    ...
                    borderColor: sector===s ? '#C9A66B' : 'var(--border)',
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(190px,1fr))', gap:8, marginBottom:32, maxHeight:380, overflowY:'auto', paddingRight:4 }}>
                {(sector ? getIndustriesBySector(sector) : INDUSTRY_OPTIONS).map(ind => (
                  <button key={ind.id} onClick={() => selectIndustry(ind.id)} style={{
                    ...
                    border: industry===ind.id ? '1.5px solid #C9A66B' : '1.5px solid var(--border)',
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))', gap:8, marginBottom:36 }}>
                {roles.map(r => (
                  <button key={r.id} onClick={() => setRole(r.id)} style={{
                    ...
                    border: role===r.id ? '1.5px solid #C9A66B' : '1.5px solid var(--border)',
                  }}>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:8, marginBottom:32 }}>
                ...
                {templates.map(tpl => tpl.id !== 'sample' && (
                  <button key={tpl.id} onClick={() => setTemplate(tpl.id)} style={{
                    ...
                    border: template===tpl.id ? '1.5px solid #C9A66B' : '1.5px solid var(--border)',
                  }}>
          ...
          {step === 4 && (
            <div>
              ...
              <div style={{ background:'rgba(1,118,211,0.04)', border:'1px solid rgba(201,166,107,0.18)', borderRadius:12, padding:'14px 18px', marginBottom:24 }}>
