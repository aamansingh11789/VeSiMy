...
export function ProcessHealthScore({ steps, compact=false }: Props) {
  ...
  // REVIEW FIX #19: null score = insufficient data (< 3 steps with CT)
  const displayScore = h.total ?? 0
  ...
  if (compact) {
    ...
    return (
      <div style={{ display:'flex', alignItems:'center', gap:6, padding:'4px 11px', background:`${h.color}12`, border:`1px solid ${h.color}35`, borderRadius:100 }}>
        ...
        <span style={{ fontSize:12, fontWeight:700, color:h.color }}>{displayScore}</span>
  ...
  return (
    <div className="card" style={{ padding:'20px' }}>
      <div style={{ textAlign:'center', marginBottom:16 }}>
        {!h.hasData ? (
          <div style={{ padding: '20px 0', color: 'var(--text3)', fontSize: 12 }}>
        ...
        <div style={{ marginTop:-8 }}>
          <div style={{ fontSize:28, fontWeight:900, color:h.color, lineHeight:1 }}>{displayScore}</div>
      ...
      {h.hasData && ([['Lead Time', h.lead_time,'var(--steel)'],['Bottlenecks',h.bottleneck,'var(--red)'],
