...
export function BetaBanner({ userId, isBeta, isLifetime, betaExpiresAt, onClaimed }: Props) {
      ...
      <div style={{ position:'relative', padding:'16px 20px', marginBottom:24,
        ...
        <div style={{ flex:1, minWidth:200 }}>
          ...
          <p style={{ fontSize:12, color:'var(--text3)', margin:0 }}>
            Upgrade to <strong style={{ color:'#0176D3' }}>Lifetime access</strong> for a one-time $99 —
        ...
        <button onClick={() => router.push('/pricing#lifetime')} style={{ padding:'9px 18px', borderRadius:8, fontWeight:700, fontSize:13, cursor:'pointer',
          background:'linear-gradient(135deg,#0a5eaa,#0176D3)', color:'var(--bg)', border:'none', whiteSpace:'nowrap' }}>
          Upgrade — $99 Lifetime
        </button>
      </div>
    )
  ...
  if (isBeta && !isLifetime) {
    ...
    const urgencyBorder = days <= 5  ? 'rgba(255,107,107,0.3)'  : days <= 10 ? 'rgba(244,166,35,0.25)' : 'rgba(1,118,211,0.22)'
    const urgencyColor  = days <= 5  ? '#FF6B6B'                : days <= 10 ? '#F4A623'               : '#0176D3'
    return (
      <div style={{ padding:'14px 18px', marginBottom:24,
        ...
        <div style={{ flex:1, minWidth:200 }}>
          <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:3 }}>
            ...
            <span style={{ fontSize:10, fontWeight:700, background:'linear-gradient(135deg,#0a5eaa,#0176D3)', color:'var(--bg)', padding:'2px 8px', borderRadius:100 }}>
              GOLD STANDARD
            </span>
          </div>
          <p style={{ fontSize:12, color:'var(--text3)', margin:0 }}>
        ...
        <button onClick={() => router.push('/pricing#lifetime')} style={{ padding:'8px 16px', borderRadius:8, fontWeight:700, fontSize:12, cursor:'pointer', whiteSpace:'nowrap',
          background:'linear-gradient(135deg,#0a5eaa,#0176D3)', color:'var(--bg)', border:'none' }}>
          Upgrade — $99 Lifetime
        </button>
      </div>
    )
    ...
    <div style={{ position:'relative', padding:'14px 18px', marginBottom:24,
      ...
      <div style={{ flex:1, minWidth:200 }}>
        ...
        <p style={{ fontSize:12, color:'var(--text3)', margin:0 }}>
          ...
          then $99 Lifetime. Your company gets a <strong style={{ color:'#0176D3' }}>33% enterprise discount</strong> forever.
        </p>
      </div>
      <Link href="/beta" style={{ textDecoration:'none', padding:'8px 16px', borderRadius:8, fontWeight:700, fontSize:12,
        background:'linear-gradient(135deg,#0a5eaa,#0176D3)', color:'var(--bg)', whiteSpace:'nowrap' }}>
        Join Early Access →
      </Link>
    </div>
  )
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 36 36" fill="none" style={{ flexShrink:0 }}>
      ...
      <path d="M4 26 L8 12 L14 20 L18 8 L22 20 L28 12 L32 26 Z"
        fill="url(#goldGrad)" stroke="#0a5eaa" strokeWidth="0.5" strokeLinejoin="round" />
      <rect x="4" y="26" width="28" height="4" rx="2" fill="url(#goldGrad)" stroke="#0a5eaa" strokeWidth="0.5" />
