...
export default function IshikawaTool({ stepId, stepName, data, onSave, onClose }: Props) {
  ...
  return (
    <Modal title={`🐟 Fishbone Diagram — ${stepName}`} onClose={onClose} onSave={handleSave}
      saveLabel={saving ? 'Saving…' : `Save Diagram (${totalCauses} cause${totalCauses!==1?'s':''})`} width={680}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr auto', gap:10, marginBottom:16 }}>
        <div>
          ...
          <input className="input" placeholder="What is the problem being analyzed? e.g. High defect rate at this step"
      ...
      {problem && (
        <div style={{ background:'rgba(212,162,8,0.04)', border:'1px solid rgba(212,162,8,0.2)', borderRadius:8, padding:'8px 14px', marginBottom:14, fontSize:12, color:'var(--text2)' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))', gap:12 }}>
        {cats.map(cat => (
          <div key={cat} style={{ background:'var(--bg)', border:'1px solid var(--border)', borderRadius:8, padding:12 }}>
            <div style={{ fontSize:11, fontWeight:700, color:'#D4A208', marginBottom:8, letterSpacing:0.5 }}>{cat}</div>
            <div style={{ display:'flex', flexDirection:'column', gap:4, marginBottom:8, minHeight:32 }}>
              {(causes[cat] || []).map((c, i) => (
                <div key={i} style={{ display:'flex', alignItems:'center', gap:6, fontSize:12, color:'var(--text2)' }}>
                  ...
                  <button onClick={() => removeCause(cat, i)} style={{ background:'none', border:'none', color:'#7070A0', cursor:'pointer', fontSize:12, padding:'0 2px', flexShrink:0 }}>×</button>
                </div>
              ))}
              {(causes[cat] || []).length === 0 && (
            ...
            <div style={{ display:'flex', gap:4 }}>
              <input className="input" style={{ fontSize:12, padding:'4px 8px', flex:1 }}
                placeholder="Add cause…"
                value={newCause[cat] || ''}
                onChange={e => setNewCause(prev => ({ ...prev, [cat]: e.target.value }))}
                onKeyDown={e => e.key === 'Enter' && addCause(cat)}
              />
              <button onClick={() => addCause(cat)} style={{ background:'rgba(212,162,8,0.15)', border:'1px solid rgba(212,162,8,0.3)', color:'#D4A208', borderRadius:6, padding:'4px 10px', cursor:'pointer', fontSize:13, flexShrink:0 }}>+</button>
            </div>
          </div>
        ))}
      </div>
